...
def test(config_path):
    ...
    results_dir = os.path.join("results", f"{opt['name']}_hazy_only")
    ...
    print("Inference mode: hazy-only")
    ...
    csv_path = os.path.join(results_dir, f"{opt['name']}_metrics.csv")
    write_csv(csv_path, rows, summary)

    print(f"\nResults on {opt['name']} (hazy-only):")
